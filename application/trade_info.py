...
def upload_to_s3(file_bytes, file_name):
    """Save a file under application/artifacts/ and return the local path (no S3)."""
    try:
        os.makedirs(ARTIFACTS_DIR, exist_ok=True)
        safe_name = os.path.basename(file_name)
        dest = os.path.join(ARTIFACTS_DIR, safe_name)
        with open(dest, "wb") as f:
            f.write(file_bytes)
        logger.info(f"saved chart locally: {dest}")
        return dest
    except Exception as e:
        logger.info(f"Error saving file locally: {e}")
        return None
